		...
		<div class="container-fluid">
			<div id="wrap">
				<div class="btn-group btn-group-xs pull-right">
					<a class="btn btn-default active" href="./readpowerfactor.php?bulbid=<?php echo $_GET['bulbid'];?>">Real-Time</a>
					<a class="btn btn-default" href="./readpowerfactorhour.php?bulbid=<?php echo $_GET['bulbid'];?>">Hour</a>
...
<?php 
	$sql="SELECT * FROM (SELECT pf, timestamp FROM poweranalyzer WHERE bulbid=".$_GET['bulbid']." ORDER BY timestamp DESC LIMIT 20)sub ORDER BY timestamp ASC";
	$result=mysql_query($sql);
	$PowerFactorReadingArray = array();
	$countPowerFactorReadings = 0;
	while($row=mysql_fetch_array($result))
	{
		$PowerFactorReadingArray[$countPowerFactorReadings]['pf'] = $row['pf'];
		$PowerFactorReadingArray[$countPowerFactorReadings]['timestamp'] = $row['timestamp'];
		$countPowerFactorReadings++;
	}
	mysql_free_result($result);
?>
<script>
