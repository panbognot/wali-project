			...
			<div class="row">
				<div class="panel panel-warning">
					...
					<div class="list-group">
						...
						<a href="./readpowerfactor.php?bulbid=<?php echo $_GET['bulbid'];?>" class="list-group-item list-group-item-warning">Power Factor</a>
		...
		<div class="container-fluid">
			<div id="wrap">
				<div class="btn-group btn-group-xs pull-right">
					<a class="btn btn-default" href="./readpowerfactor.php?bulbid=<?php echo $_GET['bulbid'];?>">Real-Time</a>
					<a class="btn btn-default active" href="./readpowerfactorhour.php?bulbid=<?php echo $_GET['bulbid'];?>">Hour</a>
					<a class="btn btn-default" href="./readpowerfactorday.php?bulbid=<?php echo $_GET['bulbid'];?>">Day</a>
					<a class="btn btn-default" href="./readpowerfactorweek.php?bulbid=<?php echo $_GET['bulbid'];?>">Week</a>
					<a class="btn btn-default" href="./readpowerfactormonth.php?bulbid=<?php echo $_GET['bulbid'];?>">Month</a>
				</div>
				<div id="chart"></div>
			</div>
		</div>
...
<?php
	...
	for ($i=0; $i<60; $i++){
		...
		$PowerFactorReadingArray[$i]['timestamp'] = $dateHourMinuteString.":00";
		//sql here
		$sql="SELECT AVG(pf) AS pf FROM poweranalyzer WHERE bulbid=".$_GET['bulbid']." AND YEAR(timestamp)=".$yearString." AND MONTH(timestamp)=".$monthString." AND DAYOFMONTH(timestamp)=".$dayString." AND HOUR(timestamp)=".$hourString." AND MINUTE(timestamp)=".$minuteString;
		$result=mysql_query($sql);
		$row=mysql_fetch_array($result);
		$RealPowerReadingArray[$i]['pf'] = $row['pf'];
		mysql_free_result($result);
		//end sql
		if (is_null($PowerFactorReadingArray[$i]['pf']))
			$PowerFactorReadingArray[$i]['pf'] = "0";
		$date->sub($interval);
	} 
			
?>
<script>
		var dataString = '<?php echo json_encode($PowerFactorReadingArray);?>';
		var objPowerFactor = eval ("("+dataString+")");
		...
		$(function () {
			$(document).ready(function() {
				...
				$('#chart').highcharts({
					...
					title: {
						text: "Power Factor (PF) for "+"<strong><?php echo $bulbsArray[$_GET['bulbid'] - 1]['name'];?></strong>",
						style: {
							color: '#000000',
					...
					series: [{
						...
						(function() {
							...
							for (i = 59; i >= 0; i--) {
								time = (new Date(objPowerFactor[i].timestamp.replace(' ', 'T') + '+08:00')).getTime();
								value = parseFloat(objPowerFactor[i].pf);
								data.push({
									x: time,
									y: value
								});
