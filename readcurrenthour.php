			...
			<div class="row">
				<div class="panel panel-warning">
					...
					<div class="list-group">
						...
						<a href="./readcurrent.php?bulbid=<?php echo $_GET['bulbid'];?>" class="list-group-item list-group-item-warning">RMS Current</a>
					</div>
				</div>
			</div>
		</div>
		...
		<div class="container-fluid">
			<div id="wrap">
				<div class="btn-group btn-group-xs pull-right">
					<a class="btn btn-default" href="./readcurrent.php?bulbid=<?php echo $_GET['bulbid'];?>">Real-Time</a>
					<a class="btn btn-default active" href="./readcurrenthour.php?bulbid=<?php echo $_GET['bulbid'];?>">Hour</a>
					<a class="btn btn-default" href="./readcurrentday.php?bulbid=<?php echo $_GET['bulbid'];?>">Day</a>
					<a class="btn btn-default" href="./readcurrentweek.php?bulbid=<?php echo $_GET['bulbid'];?>">Week</a>
					<a class="btn btn-default" href="./readcurrentmonth.php?bulbid=<?php echo $_GET['bulbid'];?>">Month</a>
				</div>
				<div id="chart"></div>
			</div>
		</div>
...
<?php
	...
	for ($i=0; $i<60; $i++){
		...
		$CurrentReadingArray[$i]['timestamp'] = $dateHourMinuteString.":00";
		//sql here
		$sql="SELECT AVG(ampere) AS ampere FROM poweranalyzer WHERE bulbid=".$_GET['bulbid']." AND YEAR(timestamp)=".$yearString." AND MONTH(timestamp)=".$monthString." AND DAYOFMONTH(timestamp)=".$dayString." AND HOUR(timestamp)=".$hourString." AND MINUTE(timestamp)=".$minuteString;
		$result=mysql_query($sql);
		$row=mysql_fetch_array($result);
		$CurrentReadingArray[$i]['ampere'] = $row['ampere'];
		mysql_free_result($result);
		//end sql
		if (is_null($CurrentReadingArray[$i]['ampere']))
			$CurrentReadingArray[$i]['ampere'] = "0";
		$date->sub($interval);
	} 
			
?>
<script>
		var dataString = '<?php echo json_encode($CurrentReadingArray);?>';
		var objCurrent = eval ("("+dataString+")");
		...
		$(function () {
			$(document).ready(function() {
				...
				$('#chart').highcharts({
					...
					title: {
						text: "RMS Current (A) for "+"<strong><?php echo $bulbsArray[$_GET['bulbid'] - 1]['name'];?></strong>",
						style: {
							color: '#000000',
					...
					series: [{
						...
						(function() {
							...
							for (i = 59; i >= 0; i--) {
								time = (new Date(objCurrent[i].timestamp.replace(' ', 'T') + '+08:00')).getTime();
								value = parseFloat(objCurrent[i].ampere);
								data.push({
									x: time,
									y: value
								});
