		...
		<div class="container-fluid">
			<div id="wrap">
				<div class="btn-group btn-group-xs pull-right">
					...
					<a class="btn btn-default active" href="./readpowerfactorweek.php?bulbid=<?php echo $_GET['bulbid'];?>">Week</a>
...
<?php
	...
	for ($i=0; $i<7; $i++){
		...
		$PowerFactorReadingArray[$i]['timestamp'] = $dateString." 00:00:00";
		//sql here
		$sql="SELECT AVG(pf) AS pf FROM poweranalyzer WHERE bulbid=".$_GET['bulbid']." AND YEAR(timestamp)=".$yearString." AND MONTH(timestamp)=".$monthString." AND DAYOFMONTH(timestamp)=".$dayString;
		$result=mysql_query($sql);
		$row=mysql_fetch_array($result);
		$PowerFactorReadingArray[$i]['pf'] = $row['pf'];
		mysql_free_result($result);
		//end sql
