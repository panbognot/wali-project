		...
		<div class="container-fluid">
			<div id="wrap">
				<div class="btn-group btn-group-xs pull-right">
					...
					<a class="btn btn-default active" href="./readreactivepowermonth.php?bulbid=<?php echo $_GET['bulbid'];?>">Month</a>
				</div>
				<div id="chart"></div>
			</div>
		</div>
...
<?php
	...
	for ($i=0; $i<31; $i++){
		...
		$ReactivePowerReadingArray[$i]['timestamp'] = $dateString." 00:00:00";
		//sql here
		$sql="SELECT AVG(var_) AS var_ FROM poweranalyzer WHERE bulbid=".$_GET['bulbid']." AND YEAR(timestamp)=".$yearString." AND MONTH(timestamp)=".$monthString." AND DAYOFMONTH(timestamp)=".$dayString;
		$result=mysql_query($sql);
		$row=mysql_fetch_array($result);
