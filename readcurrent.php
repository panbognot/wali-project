		...
		<div class="container-fluid">
			<div id="wrap">
				<div class="btn-group btn-group-xs pull-right">
					<a class="btn btn-default active" href="./readcurrent.php?bulbid=<?php echo $_GET['bulbid'];?>">Real-Time</a>
					<a class="btn btn-default" href="./readcurrenthour.php?bulbid=<?php echo $_GET['bulbid'];?>">Hour</a>
...
<?php 
	$sql="SELECT * FROM (SELECT ampere, timestamp FROM poweranalyzer WHERE bulbid=".$_GET['bulbid']." ORDER BY timestamp DESC LIMIT 20)sub ORDER BY timestamp ASC";
	$result=mysql_query($sql);
	$CurrentReadingArray = array();
	$countCurrentReadings = 0;
	while($row=mysql_fetch_array($result))
	{
		$CurrentReadingArray[$countCurrentReadings]['ampere'] = $row['ampere'];
		$CurrentReadingArray[$countCurrentReadings]['timestamp'] = $row['timestamp'];
		$countCurrentReadings++;
	}
	mysql_free_result($result);
?>
<script>
