		...
		<div class="container-fluid">
			<div id="wrap">
				<div class="btn-group btn-group-xs pull-right">
					<a class="btn btn-default active" href="./readreactivepower.php?bulbid=<?php echo $_GET['bulbid'];?>">Real-Time</a>
					<a class="btn btn-default" href="./readreactivepowerhour.php?bulbid=<?php echo $_GET['bulbid'];?>">Hour</a>
...
<?php 
	$sql="SELECT * FROM (SELECT var_, timestamp FROM poweranalyzer WHERE bulbid=".$_GET['bulbid']." ORDER BY timestamp DESC LIMIT 20)sub ORDER BY timestamp ASC";
	$result=mysql_query($sql);
	$ReactivePowerReadingArray = array();
	$countReactivePowerReadings = 0;
	while($row=mysql_fetch_array($result))
	{
		$ReactivePowerReadingArray[$countReactivePowerReadings]['var_'] = $row['var_'];
		$ReactivePowerReadingArray[$countReactivePowerReadings]['timestamp'] = $row['timestamp'];
		$countReactivePowerReadings++;
	}
	mysql_free_result($result);
?>
<script>
						...
						events: {
							load: function() {
								...
								setInterval(function() {
									...
									var feed = $.ajax({
										type: 'GET',
										url: './reactivepowerdata.php',
										data: 'bulbid='+bulbid,
										success: function(data){
											var dataString = data.split(' ');
											time = (new Date(dataString[1]+'T'+dataString[2]+'+08:00')).getTime();
											value = parseFloat(dataString[0]);
											x = time;
											y = value;
