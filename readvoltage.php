		...
		<div class="container-fluid">
			<div id="wrap">
				<div class="btn-group btn-group-xs pull-right">
					<a class="btn btn-default active" href="./readvoltage.php?bulbid=<?php echo $_GET['bulbid'];?>">Real-Time</a>
					<a class="btn btn-default" href="./readvoltagehour.php?bulbid=<?php echo $_GET['bulbid'];?>">Hour</a>
...
<?php 
	$sql="SELECT * FROM (SELECT volt, timestamp FROM poweranalyzer WHERE bulbid=".$_GET['bulbid']." ORDER BY timestamp DESC LIMIT 20)sub ORDER BY timestamp ASC";
	$result=mysql_query($sql);
	$VoltageReadingArray = array();
	$countVoltageReadings = 0;
	while($row=mysql_fetch_array($result))
	{
		$VoltageReadingArray[$countVoltageReadings]['volt'] = $row['volt'];
		$VoltageReadingArray[$countVoltageReadings]['timestamp'] = $row['timestamp'];
		$countVoltageReadings++;
	}
	mysql_free_result($result);
?>
<script>
