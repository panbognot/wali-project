			...
			<div class="row">
				<div class="panel panel-warning">
					...
					<div class="list-group">
						<a href="./readstatus.php?bulbid=<?php echo $_GET['bulbid'];?>" class="list-group-item list-group-item-warning">Status</a>
		...
		<div class="container-fluid">
			<div id="wrap">
				<div class="btn-group btn-group-xs pull-right">
					<a class="btn btn-default" href="./readstatus.php?bulbid=<?php echo $_GET['bulbid'];?>">Real-Time</a>
					<a class="btn btn-default active" href="./readstatushour.php?bulbid=<?php echo $_GET['bulbid'];?>">Hour</a>
					<a class="btn btn-default" href="./readstatusday.php?bulbid=<?php echo $_GET['bulbid'];?>">Day</a>
					<a class="btn btn-default" href="./readstatusweek.php?bulbid=<?php echo $_GET['bulbid'];?>">Week</a>
					<a class="btn btn-default" href="./readstatusmonth.php?bulbid=<?php echo $_GET['bulbid'];?>">Month</a>
				</div>
				<div id="chart"></div>
			</div>
		</div>
...
<?php
	...
	for ($i=0; $i<60; $i++){
		...
		$StatReadingArray[$i]['timestamp'] = $dateHourMinuteString.":00";
		//sql here
		$sql="SELECT AVG(stat) AS stat FROM poweranalyzer WHERE bulbid=".$_GET['bulbid']." AND YEAR(timestamp)=".$yearString." AND MONTH(timestamp)=".$monthString." AND DAYOFMONTH(timestamp)=".$dayString." AND HOUR(timestamp)=".$hourString." AND MINUTE(timestamp)=".$minuteString;
		$result=mysql_query($sql);
		$row=mysql_fetch_array($result);
		$StatReadingArray[$i]['stat'] = $row['stat'];
		mysql_free_result($result);
		//end sql
		if (is_null($StatReadingArray[$i]['stat']))
			$StatReadingArray[$i]['stat'] = "0";
		$date->sub($interval);
	} 
			
?>
<script>
		var dataString = '<?php echo json_encode($StatReadingArray);?>';
		var objStat = eval ("("+dataString+")");
		...
		$(function () {
			$(document).ready(function() {
				...
				$('#chart').highcharts({
					...
					title: {
						text: "Load Status for "+"<strong><?php echo $bulbsArray[$_GET['bulbid'] - 1]['name'];?></strong>",
						style: {
							color: '#000000',
					...
					series: [{
						...
						(function() {
							...
							for (i = 59; i >= 0; i--) {
								time = (new Date(objStat[i].timestamp.replace(' ', 'T') + '+08:00')).getTime();
								if (objStat[i].stat.localeCompare('InRange') == 0)
									value = 1.0;
								else
									value = 2.0;
								data.push({
