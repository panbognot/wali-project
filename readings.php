		...
		<div class="container-fluid">
			<div id="wrap">
				<div class="btn-group btn-group-xs pull-right">
					<a class="btn btn-default active" href="./readings.php?bulbid=<?php echo $_GET['bulbid'];?>">Real-Time</a>
...
<?php 
	$sql="SELECT * FROM (SELECT watts, timestamp FROM poweranalyzer WHERE bulbid=".$_GET['bulbid']." ORDER BY timestamp DESC LIMIT 20)sub ORDER BY timestamp ASC";
	$result=mysql_query($sql);
	$RealPowerReadingArray = array();
	$countRealPowerReadings = 0;
	while($row=mysql_fetch_array($result))
	{
		$RealPowerReadingArray[$countRealPowerReadings]['watts'] = $row['watts'];
		$RealPowerReadingArray[$countRealPowerReadings]['timestamp'] = $row['timestamp'];
		$countRealPowerReadings++;
	}
	mysql_free_result($result);		
?>
<script>
		var dataString = '<?php echo json_encode($RealPowerReadingArray);?>';
		var objRealPower = eval ("("+dataString+")");
		...
		$(function () {
			$(document).ready(function() {
				...
				$('#chart').highcharts({
					...
					title: {
						text: "Real Power (RMS) for "+"<strong><?php echo $bulbsArray[$_GET['bulbid'] - 1]['name'];?></strong>",
						style: {
							color: '#000000',
					...
					series: [{
						...
						(function() {
							...
							for (i = 0; i < 20; i++) {
								time = (new Date(objRealPower[i].timestamp.replace(' ', 'T') + '+08:00')).getTime();
								value = parseFloat(objRealPower[i].watts);
								data.push({
									x: time,
									y: value
								});
