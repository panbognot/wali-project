...
<?php
	...
	for ($i=0; $i<24; $i++){
		...
		$RealPowerReadingArray[$i]['timestamp'] = $dateHourString.":00:00";
		//sql here
		$sql="SELECT AVG(watts) AS watts FROM poweranalyzer WHERE bulbid=".$_GET['bulbid']." AND YEAR(timestamp)=".$yearString." AND MONTH(timestamp)=".$monthString." AND DAYOFMONTH(timestamp)=".$dayString." AND HOUR(timestamp)=".$hourString;
		$result=mysql_query($sql);
		$row=mysql_fetch_array($result);
