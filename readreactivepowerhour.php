			...
			<div class="row">
				<div class="panel panel-warning">
					...
					<div class="list-group">
						...
						<a href="./readreactivepower.php?bulbid=<?php echo $_GET['bulbid'];?>" class="list-group-item list-group-item-warning">Reactive Power</a>
		...
		<div class="container-fluid">
			<div id="wrap">
				<div class="btn-group btn-group-xs pull-right">
					<a class="btn btn-default" href="./readreactivepower.php?bulbid=<?php echo $_GET['bulbid'];?>">Real-Time</a>
					<a class="btn btn-default active" href="./readreactivepowerhour.php?bulbid=<?php echo $_GET['bulbid'];?>">Hour</a>
					<a class="btn btn-default" href="./readreactivepowerday.php?bulbid=<?php echo $_GET['bulbid'];?>">Day</a>
					<a class="btn btn-default" href="./readreactivepowerweek.php?bulbid=<?php echo $_GET['bulbid'];?>">Week</a>
					<a class="btn btn-default" href="./readreactivepowermonth.php?bulbid=<?php echo $_GET['bulbid'];?>">Month</a>
				</div>
				<div id="chart"></div>
			</div>
		</div>
...
<?php
	$date = new DateTime();
	$ReactivePowerReadingArray = array();
	$interval = new DateInterval('PT1M');
	for ($i=0; $i<60; $i++){
		$dateHourMinuteString = date_format($date, 'Y-m-d H:i');
		$yearString = date_format($date, 'Y');
		$monthString = date_format($date, 'm');
		$dayString = date_format($date, 'd');
		$hourString = date_format($date, 'H');
		$minuteString = date_format($date, 'i');
		$ReactivePowerReadingArray[$i]['timestamp'] = $dateHourMinuteString.":00";
		//sql here
		$sql="SELECT AVG(var_) AS var_ FROM poweranalyzer WHERE bulbid=".$_GET['bulbid']." AND YEAR(timestamp)=".$yearString." AND MONTH(timestamp)=".$monthString." AND DAYOFMONTH(timestamp)=".$dayString." AND HOUR(timestamp)=".$hourString." AND MINUTE(timestamp)=".$minuteString;
		$result=mysql_query($sql);
		$row=mysql_fetch_array($result);
		$ReactivePowerReadingArray[$i]['var_'] = $row['var_'];
		mysql_free_result($result);
		//end sql
		if (is_null($ReactivePowerReadingArray[$i]['var_']))
			$ReactivePowerReadingArray[$i]['var_'] = "0";
		$date->sub($interval);
	} 
			
?>
<script>
		var dataString = '<?php echo json_encode($ReactivePowerReadingArray);?>';
		var objReactivePower = eval ("("+dataString+")");
		...
		$(function () {
			$(document).ready(function() {
				...
				$('#chart').highcharts({
					...
					title: {
						text: "Reactive Power (VAR) for "+"<strong><?php echo $bulbsArray[$_GET['bulbid'] - 1]['name'];?></strong>",
						style: {
							color: '#000000',
					...
					series: [{
						...
						(function() {
							...
							for (i = 59; i >= 0; i--) {
								time = (new Date(objReactivePower[i].timestamp.replace(' ', 'T') + '+08:00')).getTime();
								value = parseFloat(objReactivePower[i].var_);
								data.push({
									x: time,
									y: value
								});
