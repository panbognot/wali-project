		...
		<div class="container-fluid">
			<div id="wrap">
				<div class="btn-group btn-group-xs pull-right">
					...
					<a class="btn btn-default active" href="./readingshour.php?bulbid=<?php echo $_GET['bulbid'];?>">Hour</a>
...
<?php
	...
	for ($i=0; $i<60; $i++){
		...
		$RealPowerReadingArray[$i]['timestamp'] = $dateHourMinuteString.":00";
		//sql here
		$sql="SELECT AVG(watts) AS watts FROM poweranalyzer WHERE bulbid=".$_GET['bulbid']." AND YEAR(timestamp)=".$yearString." AND MONTH(timestamp)=".$monthString." AND DAYOFMONTH(timestamp)=".$dayString." AND HOUR(timestamp)=".$hourString." AND MINUTE(timestamp)=".$minuteString;
		$result=mysql_query($sql);
		$row=mysql_fetch_array($result);
