		...
		<div class="container-fluid">
			<div id="wrap">
				<div class="btn-group btn-group-xs pull-right">
					...
					<a class="btn btn-default active" href="./readvoltageday.php?bulbid=<?php echo $_GET['bulbid'];?>">Day</a>
...
<?php
	...
	for ($i=0; $i<24; $i++){
		...
		$VoltageReadingArray[$i]['timestamp'] = $dateHourString.":00:00";
		//sql here
		$sql="SELECT AVG(volt) AS volt FROM poweranalyzer WHERE bulbid=".$_GET['bulbid']." AND YEAR(timestamp)=".$yearString." AND MONTH(timestamp)=".$monthString." AND DAYOFMONTH(timestamp)=".$dayString." AND HOUR(timestamp)=".$hourString;
		$result=mysql_query($sql);
		$row=mysql_fetch_array($result);
