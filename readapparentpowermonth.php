		...
		<div class="container-fluid">
			<div id="wrap">
				<div class="btn-group btn-group-xs pull-right">
					...
					<a class="btn btn-default active" href="./readapparentpowermonth.php?bulbid=<?php echo $_GET['bulbid'];?>">Month</a>
				</div>
				<div id="chart"></div>
			</div>
		</div>
...
<?php
	...
	for ($i=0; $i<31; $i++){
		$dateHourString = date_format($date, 'Y-m-d');
		$yearString = date_format($date, 'Y');
		$monthString = date_format($date, 'm');
		$dayString = date_format($date, 'd');
		$ApparentPowerReadingArray[$i]['timestamp'] = $dateHourString." 00:00:00";
		//sql here
		$sql="SELECT AVG(va) AS va FROM poweranalyzer WHERE bulbid=".$_GET['bulbid']." AND YEAR(timestamp)=".$yearString." AND MONTH(timestamp)=".$monthString." AND DAYOFMONTH(timestamp)=".$dayString;
		$result=mysql_query($sql);
		$row=mysql_fetch_array($result);
