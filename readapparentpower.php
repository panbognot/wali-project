		...
		<div class="container-fluid">
			<div id="wrap">
				<div class="btn-group btn-group-xs pull-right">
					<a class="btn btn-default active" href="./readapparentpower.php?bulbid=<?php echo $_GET['bulbid'];?>">Real-Time</a>
					<a class="btn btn-default" href="./readapparentpowerhour.php?bulbid=<?php echo $_GET['bulbid'];?>">Hour</a>
					<a class="btn btn-default" href="./readapparentpowerday.php?bulbid=<?php echo $_GET['bulbid'];?>">Day</a>
...
<?php 
	$sql="SELECT * FROM (SELECT va, timestamp FROM poweranalyzer WHERE bulbid=".$_GET['bulbid']." ORDER BY timestamp DESC LIMIT 20)sub ORDER BY timestamp ASC";
	$result=mysql_query($sql);
	$ApparentPowerReadingArray = array();
	$countApparentPowerReadings = 0;
	while($row=mysql_fetch_array($result))
	{
		$ApparentPowerReadingArray[$countApparentPowerReadings]['va'] = $row['va'];
		$ApparentPowerReadingArray[$countApparentPowerReadings]['timestamp'] = $row['timestamp'];
		$countApparentPowerReadings++;
	}
	mysql_free_result($result);		
?>
<script>
